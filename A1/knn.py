...
def compute_distances_two_loops(x_train, x_test):
  ...
  #print('flat shape: ', x_test[1].reshape(1,-1).shape)
  for i in range(num_train):
      train_flat = x_train[i].reshape(1,-1)
      for j in range(num_test):
          test_flat = x_test[j].reshape(1,-1)
          dists[i, j] = ((train_flat - test_flat)**2).sum()
...
def compute_distances_one_loop(x_train, x_test):
  ...
  #print('shape: ', ((x_test_flat - x_train_flat[1])**2).sum(dim=1).shape)
  for i in range(num_train):
      dists[i] = ((x_test_flat - x_train_flat[i])**2).sum(dim=1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_test_flat = x_test.reshape(num_test, -1).t()
  dists = dists + (x_train_flat**2).sum(dim=1, keepdim=True) + (x_test_flat**2).sum(dim=0, keepdim=True) - 2*torch.matmul(x_train_flat, x_test_flat)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _,k_idx = torch.topk(dists, k, dim=0, largest=False)
  #print('idx: ', k_idx)
  y_label = y_train[k_idx]
  #print('label: ', y_label)
  y_pred, _ = torch.mode(y_label, dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for i in range(num_folds):
          this_x_train = torch.cat([fold for idx, fold in enumerate(x_train_folds) if idx != i])
          this_y_train = torch.cat([fold for idx, fold in enumerate(y_train_folds) if idx != i])
          #print("shape :", this_x_train.shape, this_y_train.shape)
          myKnnClassifier = KnnClassifier(this_x_train, this_y_train)
          acc = myKnnClassifier.check_accuracy(x_train_folds[i], y_train_folds[i], k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  mean_accs = []
  k_idx = []
  for k, k_accs in k_to_accuracies.items():
      mean_accs.append(torch.tensor(list(k_accs.values()), dtype=torch.float32).mean())
      k_idx.append(k)
  
  mean_accs = torch.tensor(mean_accs, dtype=torch.float32)
  k_idx = torch.tensor(k_idx)
  #k_to_accuracies[k] = k_to_accuracies[k].values().mean()
  best_k = k_idx[torch.argmax(mean_accs)]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
