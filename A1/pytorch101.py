...
def slice_indexing_practice(x):
  ...
  even_rows_odd_cols = x[0::2, 1:0:2]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  out = (
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  row_min_idxs = y.argmin(dim=1)
  print(row_min_idxs)
  y[range(y.shape[0]),row_min_idxs] = 0;
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  # Replace "pass" statement with your code
  B = x.shape[0]
  z = torch.zeros(B, x.shape[1], y.shape[2])
  if use_loop:
...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  mu_j = x.sum(dim=0)/x.shape[1]
  #print("mu_j: ", mu_j)
  sigma_j = (x - mu_j)**2
  sigma_j = torch.sqrt( sigma_j.sum(dim=0) / (x.shape[1]-1) )
...
def mm_on_gpu(x, w):
  ...
  x_gpu = x.cuda()
  w_gpu = w.cuda()
  y = torch.matmul(x_gpu, w_gpu).to('cpu')
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
