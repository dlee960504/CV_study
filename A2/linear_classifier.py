...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # Replace "pass" statement with your code
  num_train = X.shape[0]
  WX = torch.matmul(X, W).t()   # (C, N)
  W_yj = torch.sum(W[:,y].t() * X, dim=1)   #(1, N)
  margins = torch.sum(WX - W_yj, dim=1)
  print('reg: ', reg * torch.sum(W * W))
  loss = torch.sum(margins).item() + W.shape[1] - 1 + (num_train * reg * torch.sum(W ** 2))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
